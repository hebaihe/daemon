...
int recv_file(int socketFd, char *md5, off_t *outFileSize)
{ //传出file大小,因为文件以MD5命名，所以要传入MD5
    int dataLen;
    //char buf[1000] = {0};
    ...
    //不需要接受文件名了，文件以MD5命名，直接接受大小即可
    //int fd=open(md5,O_RDWR|O_CREAT,0666);//有则打开，没有就创建，下载实现断点续传才需要这样子，上传直接用下面这种
    int fd = open(md5, O_RDWR | O_CREAT | O_EXCL, 0666);
